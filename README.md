...
2. Ввести выданный ключ доступа к библиотеке. Ключ не передаётся серверу и не
   сохраняется браузером; после перезагрузки страницы ввести его снова.
...
4. Для обновления выбрать выданный OTA-файл `.bin`, затем **Update**. Проверка
   ...
   Ползунки и консоль блокируются на время OTA.
...
**MIC** включается отдельно для диагностики реальных сэмплов и на это время
приостанавливает музыку. Если измерение занято
или устарело, страница показывает отсутствие данных. **Reset** выравнивает L/R
